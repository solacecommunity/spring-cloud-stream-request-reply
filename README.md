...
- If there is no matching patterns when execute `requestAndAwaitReply()`/`requestReply()` and `IllegalArgumentException` will be thrown.
    ...
    stream:
      requestreply:
        variableReplacements:
          "{qualifierWildcard}": ${RCS_ENV_ROLE}-${RCS_CLUSTER}
```
...
- `A requestAndAwaitReply(Q request, String requestDestination, Class<A> expectedResponseClass, Duration timeoutPeriod)`
...
- `CompletableFuture<A> requestReply(Q request, String requestDestination, Class<A> expectedClass, Duration timeoutPeriod)`
