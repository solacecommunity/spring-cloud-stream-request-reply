...
class RequestReplyMessageReplyWrappingInterceptorTests extends AbstractRequestReplyLoggingIT {
    ...
    void sendingReplyThroughFlux_shouldDoWrappingInterceptorCall() {
        ...
        requestTester.id = UUID.randomUUID().toString();
        ...
        Message<ReplyTester> replyMessage = MessageBuilder.withPayload(new ReplyTester()).build();
        Message<RequestTester> requestMessage = MessageBuilder.withPayload(requestTester).build();
        ...
        Mockito.verify(replyWrappingInterceptor, Mockito.times(1)).interceptReplyWrappingPayloadMessage(
                messageCaptor.capture(),
                eq(bindingName));
        assertEquals(requestTester, messageCaptor.getValue().getPayload());

        Mockito.verify(replyWrappingInterceptor, Mockito.times(1)).interceptReplyWrappingFinishingEmptyMessage(
                messageCaptor.capture(),
                eq(bindingName));
        assertEquals("", messageCaptor.getValue().getPayload());

        Mockito.verify(replyWrappingInterceptor, Mockito.never()).interceptReplyWrappingErrorMessage(
                any(),
                anyString());
    ...
    void sendingReplyBy_shouldDoWrappingInterceptorCall() {
        ...
        replyTester.id = UUID.randomUUID().toString();
        ...
        headerSupport.<RequestTester, ReplyTester, Function<RequestTester,ReplyTester>, RuntimeException>wrapWithBindingName(
                ...
                .apply(MessageBuilder.withPayload(new RequestTester()).build());

        // the interceptor must be called with the expected binding name:
        Mockito.verify(replyWrappingInterceptor, Mockito.times(1)).interceptReplyWrappingPayloadMessage(
                messageCaptor.capture(),
                eq(bindingName));
        assertEquals(replyTester, messageCaptor.getValue().getPayload());
